...
def query():
    ...
    selectAttributes = [s.strip() for s in "cust, sum_quant, avg_quant, state".split(',') if s.strip()]
    groupingAttributes = [g.strip() for g in "cust, state".split(',') if g.strip()]
    predicate = [p.strip() for p in "state = 'NY'".split(',') if p.strip()]

    havingCondition = [h.strip() for h in "avg_quant > 500,".split(',') if h.strip()]
    fVect = [f.strip() for f in "sum_quant, avg_quant".split(',') if f.strip()]
