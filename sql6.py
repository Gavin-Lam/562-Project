...
def query():
    ...
    selectAttributes = [s.strip() for s in "cust, 1_sum_quant, 1_avg_quant, 1_max_quant, 1_min_quant, 1_count_quant".split(',') if s.strip()]
    groupingAttributes = [g.strip() for g in "cust".split(',') if g.strip()]
    predicate = [p.strip() for p in "1.state = 'NY' and 1.year = 2020".split(',') if p.strip()]
    havingCondition = [h.strip() for h in "".split(',') if h.strip()]
    fVect = [f.strip() for f in "1_sum_quant, 1_avg_quant, 1_max_quant, 1_min_quant, 1_count_quant".split(',') if f.strip()]
    groupingVarCount = 1 
